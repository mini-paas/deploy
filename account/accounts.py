...
class Account(AccountSingleton):
    ...
    def login(self, request):
        ...
        # 手机微信端登录验证
        if self.is_weixin_visit(request) and callback_url.startswith(self._config.WEIXIN_URL_PREFIX):
            status = self.check_user_weixin_login_status(request)
            if status == 0:
                # 登录成功
                return HttpResponseRedirect(callback_url)
            # 登录失败
            return self.weixin_login_fail(request)
    ...
    def weixin_login_fail(self, request):
        """
        微信登录失败
        """
        return render_mako_context(request, self._config.WEIXIN_LOGIN_FAIL_TEMPLATE)
    ...
    def is_weixin_visit(self, request):
        ...
        @summary: 是否来自微信访问
        """
        if (self._config.RUN_MODE != 'DEVELOP' and request.path.startswith(self._config.WEIXIN_URL_PREFIX) and
                ("http://%s" % request.get_host()) == self._config.WEIXIN_BK_URL):
            return True
        return False
    ...
    def _redirect_unsingned(self, request, callback=None, jumpUrl='', redirectFieldName="refer_url"):
        """
        @summary: 跳转到登录失效页
        ...
        if self._config.RUN_MODE == 'PRODUCT':
            jumpHost = 'http://%s/' % request.get_host()
        else:
            jumpHost = 'http://%s%s' % (request.get_host(), self._config.SITE_URL)
        jumpUrl = "%saccounts/login_page/" % jumpHost if jumpUrl == '' else jumpUrl
        if callback is None:
    ...
    def _redirect_401(self, request, callback=None, jumpUrl='', redirectFieldName="c_url"):
        ...
        # ajax 登录成功后回调登录成功页面
        if callback is None:
            # 正式环境回调平台的登录成功页面，且不是独立域名
            if self._config.RUN_MODE == 'PRODUCT' and self._config.SITE_URL != '/':
                callback = "http://%s/accounts/login_success/?is_ajax=1" % request.get_host()
            else:
        ...
        if jumpUrl == '':
            ...
            jumpUrl = jumpUrl + "&" if "?" in self._config.PLAIN_LOGIN_URL else jumpUrl + '?'
    ...
    def redirectReLogin(self, request, loginUrl='', redirectFieldName=REDIRECT_FIELD_NAME):
        ...
        # 首页，开发/测试环境或者是独立域名应用， 跳登录页
        if self._config.RUN_MODE != 'PRODUCT' or request.path == '/' or self._config.SITE_URL == '/':
